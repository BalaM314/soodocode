...
export const configs = (<T extends Record<string, Record<string, [name:string, shortDescription:string, restDescription:string, defaultValue:unknown]>>>(data:T) =>
	Object.fromEntries(Object.entries(data).map(([k1, v]) =>
		...
		[K1 in keyof T]: {
			[K2 in keyof T[K1]]: Config<T[K1][K2][3]>;
		}
	}
)({
	coercion: {
